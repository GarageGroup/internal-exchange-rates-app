...
for variable in AZURE_NAME_ROOT AZURE_NAME_POSTFIX DATAVERSE_SERVICE_URL; do
...
if [[ "$(az group exists -n "$rg" -o tsv --only-show-errors)" == false ]]; then
  az group create -n "$rg" -l westeurope --tags "application=$AZURE_NAME_ROOT" "environment=$env_name" -o none --only-show-errors
fi
[[ "$(az group show -n "$rg" --query location -o tsv --only-show-errors)" == westeurope ]] || { echo 'Resource group must be westeurope' >&2; exit 1; }
...
outputs="$(az deployment group create -g "$rg" -n "exchange-rates-$env_name" --mode Incremental --template-file "$script_dir/../main.bicep" \
  --parameters nameRoot="$AZURE_NAME_ROOT" environmentName="$env_name" dataverseServiceUrl="$DATAVERSE_SERVICE_URL" \
